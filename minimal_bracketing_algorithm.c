 ...
 * 1+2*3+4 となり問題がある。これは、'*'の両隣にある'+'が'*'より優先度が低いためである。
 ...
 * 演算子を比較しながら、括弧の無駄がない式を出力する。
 * 構文解析木上では、各ノードの持つ演算子と親ノードが持つ演算子の優先度を
 * 比較することで、元の式上で隣り合った演算子の優先度を比較することが簡単にできる。
 *
 *
 * コードの概要
 * show_waste_free_expression(Operand prev_op, Node* node)関数が無駄な括弧の無い式を出力する
 * 関数である。この関数では、第一引数に現在のノードの親ノードの演算子、第二引数に
